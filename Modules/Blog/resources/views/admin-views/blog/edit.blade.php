...
@section('content')
    <div class="content container-fluid">
        ...
        <form action="{{ route('admin.blog.update') }}" method="POST" enctype="multipart/form-data" id="blog-ajax-form">
            @csrf
            <div class="card">
                <div class="card-body">
                    <div class="row gy-4 mb-4 align-items-center">
                        <div class="col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="" name="writer"  value="{{ $blog?->writer }}" class="form-control" placeholder="{{ translate('Ex') }}: {{ 'Jhon Milar' }}">
                            </div>
                            <div class="form-group mb-0">
                                ...
                                <div class="position-relative">
                                    <input type="date" name="publish_date" class="form-control cursor-pointer" value="{{ $blog->publish_date->format('Y-m-d') }}" autocomplete="off">
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            ...
                            @foreach($languages as $lang)
                                <div class="{{$lang != $defaultLanguage ? 'd-none':''}} form-system-language-form" id="{{$lang}}-form">
                                    <div class="mb-4">
                                        ...
                                        <input type="text" name="title[{{$lang}}]" class="form-control" id="title" value="{{ $defaultLanguage == $lang ? $blog->title :  $translate[$lang]['title'] ?? ''}}" placeholder="{{translate('ex').':'.translate('LUX')}}" {{$lang == $defaultLanguage ? 'required':''}}>
                                    </div>
                                </div>
                                ...
                                <div class="form-group mb-0 {{$lang != $defaultLanguage ? 'd-none':''}} form-system-description-language-form" id="{{ $lang}}-description-form">
                                    <label class="mb-2">{{ translate('Description') }}({{strtoupper($lang)}}) <span class="input-required-icon">*</span></label>
                                    <div id="description-{{$lang}}-editor" class="quill-editor">{!! $defaultLanguage == $lang ? $blog->description :  $translate[$lang]['description'] ?? '' !!}</div>
                                    <textarea name="description[{{$lang}}]" id="description-{{$lang}}" style="display:none;">{!! $defaultLanguage == $lang ? $blog->description :  $translate[$lang]['description'] ?? '' !!}</textarea>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-wrap gap-3 justify-content-end mt-4">
                        <button type="button" id="reset"
                                class="btn btn-secondary reset-form">{{ translate('reset') }}</button>
                        <button type="button" class="btn btn-outline-primary save-draft">{{ translate('Save_to_Draft') }}</button>
                        <button type="button" class="btn btn-primary publish" data-bs-toggle="modal" data-bs-target="#toggle-status-publish-modal">{{ translate('Update_Publish') }}</button>
                    </div>
                </div>
            </div>
        </form>
