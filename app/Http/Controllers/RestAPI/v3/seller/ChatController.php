    ...
    public function list(Request $request, $type)
{
    ...
    $baseQuery = Chatting::where(['seller_id' => $seller['id']]);

    if ($type == 'admin') {
        $baseQuery->where(['admin_id' => 0]);
    } else {
        $baseQuery->whereNotNull($idParam);
    }

    $total_size = (clone $baseQuery)
        ->select($idParam)
        ->distinct()
        ->get()
        ->count();

    $allChatIds = (clone $baseQuery)
        ->select('id', $idParam)
        ->latest()
        ->get()
        ->unique($idParam)
    ...
    if ($uniqueChatIds) {
        foreach ($uniqueChatIds as $uniqueChatId) {
            $query = Chatting::query();

            if ($withParam) {
                $query->with([$withParam]);
            }

            $query->where(['seller_id' => $seller['id'], $idParam => $uniqueChatId[$idParam]]);

            if ($type != 'admin') {
                $query->whereNotNull($idParam);
            }

            $userChatting = $query->latest()->first();

            if ($userChatting) {
                if ($type == 'admin') {
                    $userChatting->admin = (object)$this->getAdminUserData();
                }

                $userChatting->unseen_message_count = Chatting::where([
                    'seller_id' => $seller['id'],
                    $idParam => $userChatting[$idParam],
                    'seen_by_seller' => '0'
                ])->count();
    ...
    public function search(Request $request, $type)
{
    ...
    if ($type == 'customer') {
        ...
        $chat = Chatting::where(['seller_id' => $seller['id'], 'admin_id' => 0])->latest()->first();

        if ($chat) {
            $chat->admin = (object)$this->getAdminUserData();
    ...
    public function get_message(Request $request, $type, $id)
{
    ...
    $query->where(['seller_id' => $seller['id'], $id_param => $id])->latest();

    if (!empty($query->get())) {
        $message = $query->paginate($request->limit, ['*'], 'page', $request->offset);

        $message?->map(function ($conversation) use ($type) {
            if (!is_null($conversation->attachment_full_url) && count($conversation->attachment_full_url) > 0) {
                $attachmentData = [];
                foreach ($conversation->attachment_full_url as $attachment) {
            ...
            if ($type == 'admin') {
                $conversation->admin = (object)$this->getAdminUserData();
            }
        });

        if ($query->where($sent_by, 1)->latest()->first()) {
            $query->where($sent_by, 1)->latest()->first()->update(['seen_by_seller' => 1]);
        }
    ...
    public function send_message(Request $request, $type)
{
    ...
    if ($type == 'delivery-man') {
        ...
        event(new ChattingEvent(key: 'message_from_seller', type: 'admin', userData: $admin, messageForm: $messageForm));
    } else {
    ...
    private function getAdminUserData(): array
{
    return [
        'id' => 0,
        'f_name' => translate('admin'),
        'l_name' => '',
        'phone' => '',
        'country_code' => '',
        'image' => '',
        'image_full_url' => getWebConfig(name: 'company_fav_icon'),
        'email' => '',
    ];
}
}
