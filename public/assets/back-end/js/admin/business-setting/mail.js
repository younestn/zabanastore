        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 2) {
                    toastMagic.error(sendMail.data('error-text'));
                } else if (response.status === 1) {
                    toastMagic.success(sendMail.data('success-text'));
                } else {
                    toastMagic.info(sendMail.data('info-text'));
                }
                $('#send-mail-confirmation-modal').modal('hide');
            },
        });
    } else {
        toastMagic.error(sendMail.data('invalid-text'));
        sendMailModal.modal('hide');
    }
})
