...
<div class="position-relative nav--tab-wrapper mb-4">
    <ul class="nav nav-pills nav--tab" id="pills-tab" role="tablist">
        @foreach(EmailTemplateKey::DELIVERY_MAN_EMAIL_LIST as $key=>$value)
            <li class="nav-item">
                <a class="nav-link {{ Request::is('admin/system-setup/email-templates/delivery-man/'.$value) ?'active':'' }}"
                    href="{{ route('admin.system-setup.email-templates.view',['delivery-man',$value]) }}">
