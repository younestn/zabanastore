...
@push('css_or_js')
    ...
    <style>
        ...
        .card-chat .msg_history,
        .card-chat #chatting-messages-section {
            flex: 1 1 auto;
            min-height: 0;
        }
    </style>
...
@php
    ...
    $currentUserFieldName = isset($userType)
        ? (
            $userType == 'customer'
                ? 'user_id'
                : ($userType == 'vendor' ? 'seller_id' : 'delivery_man_id')
        )
        : 'user_id';

    $chattingPostUrl = request('type') == 'customer'
...
@endphp
    ...
    $vendorListImageSrc = !empty($chatting->seller?->image_full_url)
        ? getStorageImages(path: $chatting->seller->image_full_url, type: 'backend-profile')
        : $vendorFallbackImage;
@endphp

<img width="50"
     src="{{ $vendorListImageSrc }}"
     onerror="this.onerror=null;this.src='{{ $vendorFallbackImage }}';"
...
@endif
            ...
            <section class="col-xl-9 col-lg-8 mt-4 mt-lg-0 d-flex">
    <div class="card card-body card-chat Chat w-100 h-100 mb-0">
...
<img width="35" class="rounded-circle aspect-1" id="profile_image"
     ...
     alt="Image Description">
                                ...
                                <div class="media-body">
                                    <h5 class="profile-name mb-0" id="profile_name">{{ $lastChatUser['f_name'].' '.$lastChatUser['l_name'] }}</h5>
...
<input type="file" id="select-media"
                ...
                <div class="d-flex align-items-center justify-content-center bg-F1F7FF flex-shrink-0 radius-right-button">
                    <button class="aSend bg-primary-light outline-0 border-0 shadow-0 px-0 h-100 send-btn"
        ...
        <span id="chatting-post-url" data-url="{{ $chattingPostUrl }}"></span>
    </div>
    <span id="get-file-icon"
      data-default-icon="{{ dynamicAsset('public/assets/back-end/img/default-icon.png') }}"
