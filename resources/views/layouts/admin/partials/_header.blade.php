@php
    ...
    $langSetting = \App\Models\BusinessSetting::where('type', 'language')->first();
    $languages = $langSetting ? json_decode($langSetting['value'], true) : [];
@endphp
...
<header class="header fixed-top navbar-fixed shadow-sm bg-white">
    <div class="d-flex align-items-center justify-content-between gap-3">
        <div>
            ...
            <button type="button"
                    class="d-lg-none p-0 bg-transparent border-0"
        ...
        <div class="navbar-nav-wrap-content-right">
            <ul class="navbar-nav align-items-center flex-row gap-3">
                ...
                @if (\App\Utils\Helpers::module_permission_check('support_section'))
                    @php
                        $messageCount = \App\Models\Contact::where('seen', 0)->count();
                    @endphp
                    <li class="nav-item">
                        <a class="btn-icon position-relative"
                           href="{{ route('admin.contact.list') }}"
                           data-bs-title="{{ translate('message') }}"
                           data-bs-toggle="tooltip">
...
@push('script')
    <script>
        ...
        const advancedSearchUrl = "{{ route('admin.advanced-search') }}";
const changeLanguageUrl = "{{ route('change-language') }}";
const csrfToken = "{{ csrf_token() }}";
const currentPlaceholder = "{{ addslashes(translate('Search_or')) }}";
        ...
        $(document).ready(function () {
            ...
            let shortcutText = 'Ctrl+K';
            let isMac = false;

            if (platform.includes('mac')) {
                shortcutText = 'Cmd+K';
                isMac = true;
            } else if (platform.includes('linux') || platform.includes('win')) {
                shortcutText = 'Ctrl+K';
                isMac = false;
            }
